...
const testProductAvailability = element => {
...
const fillingCartItem = element => {
    let cartBoxItemCounter = 1;
    let productAvailabilityItem = testProductAvailability(element);

    if (productAvailabilityItem != null) {
        cartBoxItemCounter = Number(productBoxListNode[productAvailabilityItem].querySelector('div.cart-box__product-box__quantity').textContent);
        cartBoxItemCounter++;
        productBoxListNode[productAvailabilityItem].querySelector('div.cart-box__product-box__quantity').textContent = cartBoxItemCounter;
    } else {
        ...
        cartBoxItemTemplate.querySelector('div.cart-box__product-box__quantity').textContent = cartBoxItemCounter;
        ...
        itemClose.addEventListener('click', function (e) {
            productAvailabilityItem = Number(testProductAvailability(element));
            if (productAvailabilityItem != null) {
                productBoxListNode[productAvailabilityItem].parentElement.parentElement.remove();
            }
